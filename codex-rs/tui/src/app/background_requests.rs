...
pub(super) async fn upload_local_file_request(
    ...
    match &request_handle {
        AppServerRequestHandle::Remote(_) => {
            let remote_path = response.path.as_path().to_str().ok_or_else(|| {
                color_eyre::eyre::eyre!("Staged upload path must be valid UTF-8.")
            })?;
            request_handle
                .upload_file_over_sftp(remote_path, &bytes)
                .await
