...
/// Applies configured attributes as spans start so every exported span carries
/// them, not just the provider resource.
#[derive(Debug)]
...
mod tests {
    ...
    #[test]
    fn disabled_provider_clears_configured_tracestate() -> Result<(), Box<dyn Error>> {
        let mut settings = test_otel_settings();
        settings.trace_exporter = OtelExporter::OtlpHttp {
            endpoint: "http://127.0.0.1:1/v1/traces".to_string(),
            headers: std::collections::HashMap::new(),
            protocol: crate::config::OtelHttpProtocol::Json,
            tls: None,
        };
        settings.tracestate = BTreeMap::from([(
            "example".to_string(),
            BTreeMap::from([("alpha".to_string(), "one".to_string())]),
        )]);

        let configured_provider = OtelProvider::from(&settings)?.expect("otel provider");
        configured_provider.shutdown();

        assert!(OtelProvider::from(&test_otel_settings())?.is_none());
        ...
        tracing::subscriber::with_default(subscriber, || {
            ...
            let trace_context = crate::trace_context::current_span_w3c_trace_context()
                .expect("current span should have trace context");

            assert_eq!(trace_context.tracestate, None);
        });
    ...
    #[test]
    fn rejected_trace_metadata_does_not_install_global_metrics() {
        assert!(
            crate::metrics::global().is_none(),
            "global metrics already installed before provider rejection"
        );

        let mut settings = test_otel_settings();
        settings.metrics_exporter = OtelExporter::OtlpHttp {
        ...
        settings.tracestate = BTreeMap::from([(
            "Example".to_string(),
            BTreeMap::from([("s".to_string(), "2".to_string())]),
        )]);

        let Err(_) = OtelProvider::from(&settings) else {
            panic!("invalid trace metadata should fail provider initialization");
        };
